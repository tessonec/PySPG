...
class PoolSelector():
    ...
    def reload_master_info(self):
       ...
       self.cur_master  = self.connection.cursor()
       
       res = self.cur_master.execute("SELECT name, max_jobs FROM queues WHERE status = 'R'")

       for (name, max_jobs) in res:
           self.queues[name] = max_jobs

       res = self.cur_master.execute("SELECT (id, full_name, path, db_name, weight) FROM queues WHERE status = 'R'")

       self.normalising = 0.

       for (id, full_name, path, db_name, weight) in res:
           self.queues[id] = (full_name, path, db_name, weight)    



class DBSingleExecutor():
